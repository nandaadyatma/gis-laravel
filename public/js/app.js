...
function onMapClick(e) {
  ...
  if (currentSelectedLocation !== null) {
    map.removeLayer(currentSelectedLocation); // Hapus marker lama
}

// Tambahkan marker baru di lokasi klik


currentSelectedLocation = L.marker(e.latlng, {
  draggable: true,
  autoPan: true,
}).addTo(map);

currentSelectedLocation.on('dragend', function(event) {
  var marker = event.target;  // Dapatkan marker
  var position = marker.getLatLng();  
  marker.setLatLng(position).update();  
  
  // update latlng pada form
...
//current Add Location Markert
var addMarker = L.icon({
  iconUrl: 'img/current_add_location.png',
  iconSize:     [45, 75], 
  iconAnchor:   [25, 25], 
  popupAnchor:  [-10, 0]
});
